...
void countWords(const std::vector<unsigned char>& inputBytes,StartEnd startEnd,unsigned int& counter,tsl::robin_map<unsigned char, unsigned int>& wf){
    ...
    while (Pointer < Length)
    {
        ...
        if(isLetter[inputBytes[Start]]){
              if (wf.find(std::tolower(inputBytes[Start])) == wf.end())
                  wf[std::tolower(inputBytes[Start])] = 1;
              else
                  wf[std::tolower(inputBytes[Start])]++;
        }
        counter++;
    }
    if (Length-Start > 0)
...
void organizeDictIndexes(std::vector<tsl::robin_map<unsigned char,unsigned int>> wfs){
    tsl::robin_map<unsigned char, unsigned int> wfGlobal; //WordFrequency dict for creating dynamic dictIndexes
    for(auto &wf:wfs)
        for (const auto& pair : wf) {
            if(wfGlobal.find(pair.first) == wfGlobal.end())
                wfGlobal[pair.first]=pair.second;
            else
                wfGlobal[pair.first]+=pair.second;
           }
    std::vector<std::pair<unsigned char, int>> sortedPairs;
    for (const auto& pair : wfGlobal)
            sortedPairs.push_back(pair);
    auto compareBySecond = [](const std::pair<unsigned char, int>& a, const std::pair<unsigned char, int>& b) {return a.second > b.second;};
    std::sort(sortedPairs.begin(), sortedPairs.end(), compareBySecond);
    for(int i=0;i<256;i++)dictIndexes[i]=14;
    for(int i=0;i<13;i++){
        dictIndexes[sortedPairs[i].first]=i;
        dictIndexes[std::toupper(sortedPairs[i].first)]=i;
    }
    for(int i=13;i<26;i++){
        dictIndexes[sortedPairs[i].first]=25-i;
        dictIndexes[std::toupper(sortedPairs[i].first)]=25-i;
    }
}
...
std::vector<char> firstPass(const std::string fileName,int coreCount,int mode) {
    ...
    std::vector<tsl::robin_map<unsigned char, unsigned int>> wfs(coreCount); //WordFrequency dicts for all threads
    for(int i=0;i<coreCount;i++)
        countingThreads.push_back(std::thread(countWords,std::ref(fileBytes),startsandEnds[i],std::ref(sums[i+1]),std::ref(wfs[i])));

    for(auto &th:countingThreads)th.join();
    
    organizeDictIndexes(wfs);
